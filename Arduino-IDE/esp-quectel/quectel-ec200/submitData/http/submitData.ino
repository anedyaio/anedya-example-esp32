  /*
                               ...
                               Submit Data to Anedya with ESP32 and EC200U Module
                                    =======================================
  ...
  */
  ...
  void syncDeviceTimeWith_modemTime();
  ...
  void EC200U_Module_Setup() {
    ...
    sendATCommand("AT+CFUN=1,1", 3000);
    delay(10000);
  ...
  void https_context_setup() {
    ...
    sendATCommand("AT+QSSLCFG=\"seclevel\",1,0", 500);
  ...
  void syncDeviceTimeWith_modemTime() {
    ...
    for (int attempt = 1; attempt <= maxRetries; attempt++) {
      Serial.println("Attempting Time sync with modem time: Attempt " + String(attempt));

      // Send NTP request
      String response = sendATCommand("AT+QLTS=1", 5000);

      // Print the raw response for debugging
      Serial.println("Raw Modem Time: " + response);
      for(;;){}
